...
public class ProductService {
  ...
  public ResponseEntity<byte[]> getRequest(HttpServletRequest request) {
    ...
    try {
      ...
      // パラメータで部品情報Tを取得する。
      List<ProductModel> productModelList = productJdbc.select(operatorid, limit, after);

      HttpHeaders headers = new HttpHeaders();
      if (productModelList.size() == limit) {
        // 100件取得した場合、最後のレコードの更新日を使って次があるか確認する。
        ProductModel lastProduct = productModelList.get(productModelList.size() - 1);
        List<ProductModel> list = productJdbc.select(operatorid, limit, lastProduct.getModifiedAt());
        if (!list.isEmpty()) {
